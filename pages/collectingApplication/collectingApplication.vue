		...
		<view class="container">
			<view class="search-box margin-bottom-20">
				<view class="tabs-wrap">
					<uni-search-bar placeholder="请输入领用单据号" bgColor="#EEEEEE" v-model="useNo" @confirm="search" />
				</view>
			...
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view class="pro-recorder">
					<view class="pro-list" :style="[{animationDelay: (index + 1)*0.05 + 's'}]"
						...
						<view class="pro-top">
							<view class="topleft">{{item.useNo}}</view>
							<view class="topright" :class="item.isApproval_dictText == '未出库' ? 'red' :'green'">{{item.isApproval_dictText}}</view>
						</view>
						<view class="proItem">
							<view class="itemleft">领用人</view>
							<view class="itemright">{{item.createName || '无'}}</view>
						</view>

						<view class="proItem">
							<view class="itemleft">领用部门</view>
							<view class="itemright">{{ item.deptName }}</view>
						</view>
						<view class="proItem">
							<view class="itemleft">使用车间/产线</view>
							<view class="itemright">{{ item.usrProject ||'无' }}</view>
						</view>
						<view class="proItem">
							<view class="itemleft">创建日期</view>
	...
	export default {
		...
		data() {
			return {
				...
				msg2Count: "",
				msg2Title: "",
				urls: "/ep/epUserApplication/list",
				listTouchStart: 0,
				modalName: null,
