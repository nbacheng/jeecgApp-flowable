<template>
	<view class="bg-white">
		<cu-custom bgColor="bg-gradual-blue" :isBack="true" backRouterName="indexs">
			...
			<block slot="content">备件台账</block>
		</cu-custom>
		<view class="container">
			<view class="search-box margin-bottom-20">
				<view class="tabs-wrap">
					<uni-search-bar placeholder="请输入物料名称" bgColor="#EEEEEE" v-model="name" @confirm="search" />
				</view>
			...
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :top="top" @down="downCallback" @up="upCallback">
				<view class="pro-recorder">
					<view class="pro-list" :style="[{animationDelay: (index + 1)*0.05 + 's'}]"
						...
						<view class="center">
							<view class="pro-top">
								<view class="topleft">{{item.name}}</view>
								<!-- <view class="topright" :class="item.isApproval_dictText == '未出库' ? 'red' :'green'">{{item.isApproval_dictText}}</view> -->
							</view>
							<view class="proItem">
								<view class="itemleft">物料编码:</view>
								<view class="itemright">{{ item.code }}</view>
							</view>
							<view class="proItem">
								<view class="itemleft">库存:</view>
								<view class="itemright">{{ item.number }}</view>
							</view>
						</view>
					</view>
