	...
	export default {
		...
		data() {
			return {
				...
				msg2Count:"",
				msg2Title:"",
				newUrl:"/sys/sysAnnouncementSend/getMyAnnouncementSend",
		...
		methods: {
			...
			upCallback(page) {
				...
				if(keyword == 0){
					this.$http.get(this.newUrl,{params:{pageNo: page.num, pageSize:page.size,msgCategory: '1'}}).then(res=>{
				...
				if(keyword == 1){
					this.$http.get("/sys/annountCement/listByUser",{params:{pageNo: page.num,pageSize: page.size,msgCategory: '2'}}).then(res=>{
						//联网成功的回调,隐藏下拉刷新和上拉加载的状态;
						this.announcement2 = res.data.result.anntMsgList
						this.mescroll.endSuccess(this.announcement2.length,this.msgCount);
						
						//设置列表数据
						  if (res.data.success) {
							  console.log("res sys",res.data)
							 this.msg2Count = res.data.result.anntMsgTotal
							 this.msg2Title = "通知(" + res.data.result.anntMsgTotal + ")";
