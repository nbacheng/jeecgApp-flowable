<template>
    <view>
		...
		<!-- 展示部门和用户 -->
		<view v-if="memberList.length > 0" class="cu-list menu-avatar sm-border" :style="[{marginTop:'calc('+CustomBar+ 'px + 10px)'}]">
		...
		<view v-else class="cu-list menu-avatar sm-border" :style="[{marginTop:'calc('+CustomBar+ 'px + 10px)'}]">
			<view class="text-center text-lg"><text>共{{ memberTotal}}人</text></view>
		</view>
    </view>
</template>
    ...
    export default {
        ...
        methods: {
			...
			avatarHandler(item){
				...
				if(avatar){
					let url = api.getFileAccessHttpUrl(avatar);
					item.avatar = url
				}else{
					if(item.sex=='2'){
