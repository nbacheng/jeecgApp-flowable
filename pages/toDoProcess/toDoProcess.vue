				...
				<view class="pro-recorder">
					<view class="pro-list" :style="[{animationDelay: (index + 1)*0.05 + 's'}]"
						...
						<view class="pro-top">
							...
							<view class="topright"><text>处理</text></view>
						</view>
						<view class="proItem">
	...
	export default {
		...
		data() {
			return {
				...
				msg2Count: "",
				msg2Title: "",
				urls: "/flowable/task/todoListNew",
				listTouchStart: 0,
				modalName: null,
		...
		methods: {
			...
			goInfo(index) {
				...
				// #ifdef MP-WEIXIN
				  let platform = "weixin";
				// #endif
				let urls = urlsHeader + "/flowable/mtask/record/index?procInsId="+row.procInsId +"&deployId="+row.deployId+"&taskId="+row.taskId+"&businessKey="+row.businessKey +"&nodeType=" +row.nodeType +"&category="+row.category+"&finished=true"+"&username=" +name+"&types=999" + "&platform=" + platform;
