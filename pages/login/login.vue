	...
	<view class="zai-box">
		<scroll-view scroll-y class="page">
			<view class="text-center" :style="[{animation: 'show ' + 0.4+ 's 1'}]">
				<image src="https://jngc.oss-cn-beijing.aliyuncs.com/images/sales/sales_1665277406307.png"
					mode='aspectFit' class="zai-logo "></image>
				<view class="zai-title text-shadow ">欢迎登陆</view>
			</view>
				...
				this.mLogin(loginParams).then((res) => {
					...
					if (res.data.success) {
						...
						this.$http.get('/sys/permission/getUserPermissionByToken', {
							...
							uni.setStorage({
								key: "menu",
								data: res.data.result.menu
							})
							this.$tip.success('登录成功!')
							if (getApp().globalData.id != '') {
								this.$Router.replaceAll({
									name: 'epPointInspection'
								})
							} else {
								this.$Router.replaceAll({
									name: 'index'
...
<style>
	...
	.zai-box {
		...
		position: relative;
		height: 100%;
		background: linear-gradient(-3deg, #F4F3F3, #C8D9FF);
	}
