<template xlang="wxml">
	<view class="tki-tree">
		...
		<view class="tki-tree-cnt" :class="{'show':showTree}">
			<view class="tki-tree-bar flex-column-b">
			...
			<view class="tki-tree-view ">
				<scroll-view class="tki-tree-view-sc" :scroll-y="true">
					<block v-for="(item, index) in treeList" :key="index">
						<view class="tki-tree-item flex-column-b" :style="[{
			...
			_treeItemSelect(item, index) {
				...
				if (item.rank > 0) {
					...
					item.parentId.forEach((pid, indexP) => {
						...
						if (this.childNums[indexP] === undefined) {
							this.childNums[indexP] = 1
						} else if (this.childNums[indexP] < childNum) {
							this.childNums[indexP]++
						}
					})
